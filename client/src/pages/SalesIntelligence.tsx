...
export default function SalesIntelligence() {
  ...
  const productsQuery = trpc.analytics.topProducts.useQuery();
  ...
  return (
    <div className="space-y-7 pb-6">
      ...
      <section className="quantico-card overflow-hidden"><div className="flex items-center justify-between border-b border-[#e4eaea] px-5 py-5 sm:px-6"><SectionTitle eyebrow="Produtos" title="Receita por produto ou serviço" /><span className="text-xs text-[#778587]">Dados agregados por organização</span></div>{productsQuery.isLoading ? <div className="p-6 text-sm text-[#627485]">A preparar o desempenho por produto…</div> : productsQuery.data?.length ? <div className="overflow-x-auto"><table className="w-full min-w-[560px] text-left"><thead className="bg-[#fafcfc]"><tr className="text-[10px] font-bold uppercase tracking-[0.1em] text-[#849193]"><th className="px-6 py-3.5">Produto</th><th className="px-4 py-3.5 text-right">Vendas</th><th className="px-6 py-3.5 text-right">Receita</th></tr></thead><tbody>{productsQuery.data.map((product) => <tr key={product.productName} className="border-t border-[#edf1f1]"><td className="px-6 py-4 text-sm font-semibold text-[#263233]">{product.productName}</td><td className="px-4 py-4 text-right font-mono text-sm text-[#546777]">{product.salesCount.toLocaleString("pt-AO")}</td><td className="px-6 py-4 text-right text-sm font-semibold text-[#176eae]">{product.totalRevenue.toLocaleString("pt-AO")} Kz</td></tr>)}</tbody></table></div> : <div className="p-6 text-sm leading-6 text-[#627485]">Mapeie a coluna <strong>Produto</strong> em Conectores e campos para ver o desempenho real do seu catálogo.</div>}</section>
